...
# 在测试ToyModel的使用使用函数来产生数据，如果是gpt那么需要使用get_batch函数
def make_toy_data(device, total_bs=8, in_dim=10, out_dim=5):
    x = torch.randn(total_bs, in_dim, device=device)
    y = torch.randn(total_bs, out_dim, device=device)
    return x, y

def sync_model_from_rank0(model: nn.Module):
...
def main():
    ...
    mp.spawn(
        run_naive_ddp,
        args=(args.world_size, args.backend, model_fn, args.num_steps, val_data, args.batch_size, 
              args.context_length, args.seed, args.deterministic, args.use_nsys),
