...
from cs336_systems.optmizer_shard.ddp_optimizer import ShardedOptimizer
...
def run_naive_ddp(rank, world_size, backend, model_fn, num_steps, val_data, batch_size, context_length,
                  base_seed, deterministic, use_nsys, bucket_size_mb, use_sharded_optimizer):
    device = _setup_process_group(rank, world_size, backend=backend)
    seed_everything(base_seed, deterministic=deterministic)
    model = model_fn().to(device)
    torch.cuda.synchronize()
    if rank == 0:
        print("After model initialization(GB):", torch.cuda.max_memory_allocated(device) / 1024**3)
    if bucket_size_mb:
    ...
    if use_sharded_optimizer:
        optimizer = ShardedOptimizer(model.parameters(), AdamW, lr=optimizer.defaults['lr'], weight_decay=optimizer.defaults['weight_decay'])
    # warm up
    ...
    for step in range(num_steps):
        ...
        if rank == 0 and step == 0:
            print("Directly before optimizer step(GB):", torch.cuda.max_memory_allocated(device) / 1024**3)
        optimizer.step()
        torch.cuda.synchronize()
        if rank == 0 and step == 0:
            print("Directly after optimizer step(GB):", torch.cuda.max_memory_allocated(device) / 1024**3)
        if step == 0 and use_sharded_optimizer:
            print(f"Rank {rank}, Optimizer state memory usage(GB): {optimizer.occupied_memory / 1024**3:.4f} GB")
        if backend == "nccl":
            torch.cuda.synchronize()
...
def main():
    ...
    mp.spawn(
        ...
        args=(args.world_size, args.backend, model_fn, args.num_steps, val_data, 
              ...
              args.use_nsys, args.bucket_size_mb, args.use_sharded_optimizer),
